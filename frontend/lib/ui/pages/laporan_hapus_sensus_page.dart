...
import '../../data/models/laporan_hapus_sensus_model.dart';
...
class LaporanHapusSensusPage extends StatefulWidget {
  final bool showAppBar;
  
  const LaporanHapusSensusPage({super.key, this.showAppBar = true});

  @override
  State<LaporanHapusSensusPage> createState() => _LaporanHapusSensusPageState();
}

class _LaporanHapusSensusPageState extends State<LaporanHapusSensusPage>
    with SingleTickerProviderStateMixin {
  late TabController _tabController;
  List<LaporanHapusSensusModel> _laporan = [];
  ...
  Future<void> _fetchLaporan() async {
    ...
    try {
      final repo = context.read<LaporanHapusSensusRepository>();
      ...
      setState(() {
        _laporan = List<LaporanHapusSensusModel>.from(data['list']);
  ...
  Future<void> _setujui(LaporanHapusSensusModel laporan) async {
    try {
      final repo = context.read<LaporanHapusSensusRepository>();
      ...
      await _fetchLaporan();
      if (mounted) {
        CustomSnackbar.showSuccess(context, 'Laporan disetujui. Sensus telah dihapus.');
      }
    } catch (e) {
      if (mounted) {
        CustomSnackbar.showError(context, '$e');
  ...
  Future<void> _tolak(LaporanHapusSensusModel laporan) async {
    try {
      final repo = context.read<LaporanHapusSensusRepository>();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: widget.showAppBar ? AppBar(
        ...
        elevation: 0,
        title: const Text('Laporan Sensus',
  ...
  Widget _buildCard(LaporanHapusSensusModel laporan) {
    String keretaName = laporan.sensusId != null 
        ? 'Sensus #${laporan.sensusId}'
        : 'Sensus (Dihapus)';
    String rute = '-';

    if (laporan.sensus != null) {
      final lokomotif = laporan.sensus!['lokomotif'];
      if (lokomotif != null) {
        keretaName = '${lokomotif["tipe_model"]} ${lokomotif["seri_model"]}';
      }
      final kereta = laporan.sensus!['kereta'];
      if (kereta != null) {
        rute = '${kereta["rute_awal"]} - ${kereta["rute_akhir"]}';
      }
    }
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    Container(
                      ...
                      child: const Icon(Icons.assignment_turned_in, color: Color(0xFF153D77), size: 24),
                    ),
                    const SizedBox(width: 12),
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Row(children: [
                            const Icon(Icons.route_outlined, size: 13, color: Color(0xFF94A3B8)),
                ...
                Container(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      const Text('Alasan Penghapusan Sensus',
